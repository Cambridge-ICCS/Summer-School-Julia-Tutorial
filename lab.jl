...
# ╔═╡ 72aff41c-0c84-11ef-2147-8dbe04566cbb
begin
	using NeuralPDE, Unitful, Lux, DifferentialEquations, Optimization
	using CSV, DataFrames, Statistics, Plots, NaNStatistics
	using ModelingToolkit: Interval
	import PlotlyJS, OptimizationOptimJL, LineSearches

	plotlyjs()
end

# ╔═╡ bdc4d038-c78d-4834-a47a-716a53b37777
begin
...
# ╔═╡ f36955d2-981f-454a-b46f-2d05ff86893f
begin
...
# ╔═╡ 22b2d204-8a55-4cca-bb60-dac4ae39c46f
# Task 1: fit a polynomial to the Keeling curve
begin
	CCO2(t) = features(t .- 1850) * CO2_params
...
# ╔═╡ 9cb638fb-53eb-4b26-b9f7-8de556985b4f
begin
	years = 1850:2030
	let df = CO2_historical_data
		plot(df[:, "Year"] , df[:, "CO2"], 
			 label="Mauna Loa CO₂ data (Keeling curve)")
		plot!(years, CCO2(years), label="Fitted", legend=:bottomright)
	end
	title!("CO₂ observations and fit")
end

# ╔═╡ 0e14d5a7-a2b3-4994-adf1-05cb91ee3583
begin
	@parameters t α a S A B C
	@variables T(t) RC(t)

	absorbed_solar_radiation = (1-α)S/4
	# A = G(t₀) - G′(t₀)t₀
	# B = -G′(t₀)
	# outgoing_thermal_radiation = G(t₀) + G′(t₀)*(t-t₀)
	...
	eqs = [
		...
		greenhouse_effect,
		RC ~ CCO2(t+1850) / CCO2(1850)
		# RC ~ 1 + (t/220)^3
	]
end

# ╔═╡ c18605d7-9943-4229-a45e-3ab6837aabe3
@mtkbuild sys = ODESystem(eqs, t)

# ╔═╡ 45231cbd-26c4-4b53-ba93-2c1b24009e20
begin
	ini = [T => 14.0]
	ps = [
		a => 5.0, 
...
# ╔═╡ 4a005208-1678-4592-a8e6-51c8bd4b76a1
begin
...
# ╔═╡ 255ebc17-1795-452c-8b31-f454ca8f3d35
begin
...
# ╔═╡ 146c10d2-ee64-44cd-bd39-58814dc6eb20
md"""We have not taken into account other greenhouse gases and feedback factors such as water vapour."""

# ╔═╡ af01d5d8-69fe-4e07-85fb-8e973eb320be
md"""# 2D advection-diffusion model"""

# ╔═╡ f500a631-7b83-4e16-bd05-283d8d53f87d
begin
	@parameters x y κ u(..) v(..)
	@variables Y(x,y,t)

	Dt, Dx, Dy = Differential.([t, x, y])
	Dxx, Dyy = Dx^2, Dy^2

	eqn = Dt(Y) ~ Dx(Y)u(x,y) + Dy(Y)v(x,y) + (Dxx(Y) + Dyy(Y))κ
	...
	@named pde_system = PDESystem(eqn, bcs, domains, [x, y, t], [Y])
end

# ╔═╡ e3c5ad2a-f3c9-40c7-99e8-bae6bcfe97b2
nn = Lux.Chain(Dense(3, 16, Lux.relu), Dense(16, 16, Lux.relu), Dense(16, 1))

# ╔═╡ 2f046b2f-bc99-4dbc-8683-a276232f3b61
begin  # Physics-informed Neural PDE
...
# ╔═╡ 18f827ba-17e6-43d9-8731-27325b00f612
# begin
# 	# Optimizer
...
# 	res = solve(problem, opt, maxiters = 1000, callback = callback)
# 	phi = discretization.phi
# end

# ╔═╡ f8e2bbc8-509c-490e-adb1-0a366289b9ed
...
# ╔═╡ b480f521-16e5-49e5-935b-6958580a6ec2
...
manifest_format = "2.0"
project_hash = "2383706895e6dde1461fa2adf2c4db3dbc044b04"
