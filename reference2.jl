...
cloud_resolution_possible_at = let
	logC2020 = 4 * log2(700)
	logC(t) = logC2020 + (t-2020)/2
	logCtarget = 4 * log2(4e4)
	findfirst([logC(2020+t) > logCtarget for t in 1:100]) + 2020
end
...
# ╔═╡ e825f530-95d0-11eb-0471-374a9c043072


# ╔═╡ f24e8570-2e6c-11eb-2c21-d319af7cba81
function absorbed_solar_radiation(T::Array{Float64,2}, model::RadiationOceanModel)
	let grid_S = S_at.(model.grid.y, grid=model.grid, S_mean=model.params.S_mean)
		grid_S .* (1 .- α(T, model)) ./ (4 * model.params.C)
	end
end
...
function outgoing_thermal_radiation(T::Array{Float64,2}, model::RadiationOceanModel)
	return outgoing_thermal_radiation(T, C=model.params.C,
		A=model.params.A, B=model.params.B)
end
...
radiation_sim = let
	...
	params = RadiationOceanModelParameters(α0=0.2)
...
The low temperature stability point is about -42°C. I found that any temperature lower than -2.57°C will converge to this equilibrium. When the temperature is higher than -2.56°C, the temperature will rise up to about 5.8°C. Interstingly, if the init temperature is around -2.56°C, the temperature will get lower first and then rise up to the equilibrium. However, if I start from 50°C, the temperature will decrease to about 24.2°C and stay there. Like before, there is a tipping point between two equilibriums: 5.8 and 24.2. I found the tipping point is between 17.76°C-17.77°C. If the initial temperature is close to this tipping point, the change of temperature will also take a detour like in the case of the other tipping point.
"""
...
A single very low temperature stable point, about -70°C.
"""

# ╔═╡ bed2c7e0-2d15-11eb-14e3-93f5d1b6f3a1
S_2000_stable_states = md"""
A single very high temperature stable point, about 108°C.
"""
...
# ╔═╡ 2cd735c2-9600-11eb-0c79-27f2da7e6d83
...
function eq_T(S, T_init_value)
	...
	sim = ClimateModelSimulation(model, copy(T_init), Δt)
