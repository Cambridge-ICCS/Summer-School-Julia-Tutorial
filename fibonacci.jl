...
# ╔═╡ cdfbb64c-4dca-4d59-a7a9-7d4a58c0d337
...
# ╔═╡ ac43a2a3-1f92-487f-82f5-f7f6562436e5
md"Time and space complexities grow exponentially!"

# ╔═╡ 249e6b31-c4e0-432f-860b-c53248da91f4
md"### Optimize the recursive function with memoization"

# ╔═╡ 3bc04564-6a38-4103-9402-347527a7698c
md"Or use the `@memoize` macro in the 'Memoize.jl' package."
...
# ╔═╡ 3c31b603-7848-4a04-a339-0facd7f9f41c
md"""### Use linear algebra to solve the linear recurrence"""

# ╔═╡ 778f1cdd-48da-4344-9207-90dd9e64f48f
names(Symbolics)

# ╔═╡ 1882fd5b-bb08-424b-be34-a39d3db34841
let x = Symbolics.variable("x")
	@show typeof(x)
	y = [1, x, x^2]
	@show typeof(y)
	eq = Equation(y ⋅ [-3, 2, 1], 0)  # or dot(a, b)
	eq, solve_single_eq(eq, x)
end

# ╔═╡ 789b56c6-7fe9-49aa-b27a-7a7b5add5285
mutable struct LinearRecurrence{D}
	coefs::NTuple{D, <:Number}
	inis::NTuple{D, <:Number}
	formula::Num
	bind::Function

	function LinearRecurrence(coefs, inis)
		D = length(coefs)
		@assert length(inis) == D
		xs = [Symbolics.variable("x_$i") for i in 1:D]
		cs = [Symbolics.variable("c_$i") for i in 1:D]
		n = Symbolics.variable(:n)
		formula = dot(cs, xs .^ n)
		bind(; roots=xs, coefs=cs, val=n) = 
			Dict([zip(xs, roots)..., zip(cs, coefs)..., n => val])
		new{D}(Tuple(coefs), Tuple(inis), formula, bind)
	end

	(lr::LinearRecurrence)(n) = substitute(lr.formula, lr.bind(val=n)).val
end

# ╔═╡ ee8d8a8a-be48-4282-91da-fc620cc04e18
function solve!(lr::LinearRecurrence{D}) where {D}
	m = vcat(collect(lr.coefs)', I(D)[1:end-1, :])
	roots = eigvals(m)
	basis = [r ^ d for d in 1:D, r in roots]
	coefs = basis \ collect(lr.inis)
	lr.formula = substitute(lr.formula, lr.bind(roots=roots, coefs=coefs))
end

# ╔═╡ d8992408-ed9d-4113-9aee-46c228f82465
fib_lr = LinearRecurrence([1, 1], [1, 1])

# ╔═╡ 06c11c75-589e-4ca3-831a-fcf6b6b2dc8e
solve!(fib_lr)

# ╔═╡ 89773b74-79bc-407b-ae3f-5eae3a43c471
@time @. fib_lr(1:32) |> round |> BigInt

# ╔═╡ ae794ddc-9620-4fa9-b924-77beb548c712
md"### Call a function in a C-exported shared library"

# ╔═╡ f4877904-acf8-457c-87db-df7ccd79b8e8
"Call the Fibonacci function in libgmp in C."
function fib_c(n)
    z = BigInt()
...
manifest_format = "2.0"
project_hash = "34a6a964d0d711bea27992fa80dc3f5da32899be"
