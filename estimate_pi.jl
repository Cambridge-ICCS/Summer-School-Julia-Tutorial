...
# ╔═╡ 5fa51d52-7725-4659-bdb7-c10c1dfd4ce5
md"### Estimate π using fixed point iteration

If converges, the limit of ``f(x), f(f(x)), \ldots`` is a fixed point ``x_0`` of ``f`` , i.e. ``f(x_0) = x_0``. 

``\pi`` is a fixed point of ``sin(x) + x``, since ``sin(\pi) + \pi = \pi``."

# ╔═╡ e8567d8a-6e45-4d8a-a25d-a55b4da1c522
@time nfold(x -> sin(x) + x, 5)(1) |> pi_err
