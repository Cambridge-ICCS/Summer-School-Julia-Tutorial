...
md" Let us compute some machine epsilon, there exists a largest number, which we call ϵ such that 1 + x = 1 for all abs(x)< ϵ"
...
md"Values of abstract types cannot be constructed directly, `example: Number` says we can do operations like `+`, `-`, `*`, and `/` with corresponding values."
...
md"We can put all these ideas together to use in the energy balance model that can be found in this working directory."
